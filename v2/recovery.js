export function isRecoveryLocation(location){return new URLSearchParams(location.search).get('recovery')==='1'||new URLSearchParams(location.hash.slice(1)).get('type')==='recovery';}
...
export async function mountRecovery(client,root){
 ...
 if(error||!data.session||!verified){root.innerHTML='<main class="auth"><div class="card"><h1>Odkaz nie je platný</h1><p>Odkaz už mohol byť použitý alebo vypršal. Vyžiadaj si nový.</p><a href="./">Späť na prihlásenie</a></div></main>';return;}
