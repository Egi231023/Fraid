-- Protected read for scheduled reports; no new grants or table changes.
begin;
...
begin
 ...
 elsif action='report_data' then
  return jsonb_build_object(
   'people',(select coalesce(jsonb_agg(to_jsonb(person) order by person.id),'[]'::jsonb) from public.fraid_v2_people person),
   'records',(select coalesce(jsonb_agg(to_jsonb(record) order by record.kind,record.id),'[]'::jsonb) from public.fraid_v2_records record where record.kind in ('entries','wages','corrections','stock'))
  );
 elsif action='claim' then
  insert into fraid_private.deliveries(key,status,lease_until,report) values(p->>'key','processing',now()+interval '10 minutes',p->'report')
  on conflict(key) do update set status='processing',attempts=fraid_private.deliveries.attempts+1,lease_until=now()+interval '10 minutes',updated_at=now()
